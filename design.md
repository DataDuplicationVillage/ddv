...
* Kiosk web-frontend authentication based on scanning disk tags (Disk IDs), which are joined with the DiskHavings entries to find the corresponding owner ID. Kiosk users then see all transfer entries corresponding to their owner ID.
* Based on Django
* Tracks disk assignments to owners, who make diskhavings entries representing a disk transfer
* Admin users can add, read, modify, and delete all entries in all tables
* Volunteer users can add and modify disks, datasource, owners, and diskhavings entries.
* Regular users can read the diskhavings entries corresponding to their owner ID.
