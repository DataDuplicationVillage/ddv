...
class DiskGenerateQRForm(forms.Form):
    MODE_CHOICES = [
        ('new', 'New Disk + New DiskHaver Pair'),
        ('existing', 'Add Disk to Existing DiskHaver'),
    ]
    ...
    def clean(self):
        ...
        if mode == 'existing':
            ...
            if not existing_disk_id:
                self.add_error('existing_disk_id', 'Required to lookup existing DiskHaver.')
            else:
                history = DiskHavings.objects.filter(disk_id=existing_disk_id).order_by('-when')
                if not history.exists():
                    self.add_error('existing_disk_id', f'Disk {existing_disk_id} has no history. Cannot determine DiskHaver.')
                else:
